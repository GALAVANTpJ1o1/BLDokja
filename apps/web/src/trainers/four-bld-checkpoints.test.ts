...
import { gradeSwapSetup, sameLook } from "./effects";
import { fourLetterItems, fourMemo, fourParityItems, fourSetupItems, fourTraceItems, gradeFourMemo } from "./four-bld-checkpoints";
  ...
  it("the table's setup is accepted for every r2 and U2 target, and so is any other setup that works", async () => {
    ...
    for (const dataset of [r2Wings, u2Centres]) {
      for (const item of fourSetupItems(reader, createRng(dataset.id), dataset, 30)) {
        ...
        expect(gradeSwapSetup(puzzle, dataset, item.target, item.answer).correct, `${dataset.id} ${item.target}`).toBe(true);
        // No setup at all only works on the swap slot itself, which these items leave out.
        expect(gradeSwapSetup(puzzle, dataset, item.target, "").correct).toBe(false);
      }
    }
    ...
    expect(gradeSwapSetup(puzzle, u2Centres, "Lub", "2F 2U2 2F' D").correct).toBe(true);
    expect(gradeSwapSetup(puzzle, u2Centres, "Lub", "U 2F 2U2 2F'").correct).toBe(false);
  });

  it("refuses setups that damage the cube, like the dataset's own tempting shortcuts", async () => {
    const puzzle = await loadPuzzle("4x4x4");
    const { r2Wings } = algDatasets();
    for (const tempting of r2Wings.tempting.slice(0, 10)) expect(gradeSwapSetup(puzzle, r2Wings, tempting.target, tempting.setup).correct, `${tempting.target} ${tempting.setup}`).toBe(false);
    expect(gradeSwapSetup(puzzle, r2Wings, "UBl", "not a move").reason).toBe("unreadable");
  });
