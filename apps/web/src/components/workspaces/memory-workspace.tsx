...
export function MemoryWorkspace() {
  ...
  const savePair = (event: SyntheticEvent) => {
    ...
    if (first === undefined || second === undefined || extra !== undefined || word.trim() === "") { setMessage(copy.memory.invalidPair); return; }
    void perform(async () => {
      const at = nowIso();
  ...
  return <div className="flex flex-col gap-8">
    ...
    <section className="flex flex-col gap-4" aria-label={copy.memory.palace}>
      ...
      <form className="flex flex-col gap-4" onSubmit={(event) => { event.preventDefault(); void perform(() => update({ memoryPalaces: [...(stored?.memoryPalaces ?? []).filter((item) => item.id !== palace.id), palace] })); }}>
        ...
        <ol className="flex flex-col gap-4">{palace.locations.map((location, index) => <li className="grid gap-3 md:grid-cols-[1fr_2fr_auto] border-b border-rule pb-4" key={location.id}>
          ...
          <label className="t-ui flex flex-col gap-2">{copy.memory.prompt}<textarea className="field" rows={2} maxLength={1000} value={location.prompt} onChange={(event) => { setPalace({ ...palace, locations: palace.locations.map((item) => item.id === location.id ? { ...item, prompt: event.target.value } : item) }); }} /></label>
    ...
    <section className="flex flex-col gap-4 border-t border-rule pt-6" aria-label={copy.memory.pairs}><h2 className="t-heading">{copy.memory.pairs}</h2>
      <form onSubmit={savePair} className="grid gap-4 md:grid-cols-2">
        <label className="t-ui flex flex-col gap-2">{copy.memory.pair}<input className="field mono" list="saved-pair-ids" value={pairId} onChange={(event) => { selectPair(event.target.value.normalize("NFC").toLocaleUpperCase("en-GB")); }} required maxLength={20} /><datalist id="saved-pair-ids">{pairs.map((pair) => <option key={pair.id} value={pair.id} />)}</datalist></label>
        <label className="t-ui flex flex-col gap-2">{copy.memory.word}<input className="field" required maxLength={1000} value={word} onChange={(event) => { setWord(event.target.value); }} /></label>
    ...
    <section className="flex flex-col gap-4 border-t border-rule pt-6" aria-label={copy.memory.story}><h2 className="t-heading">{copy.memory.story}</h2>
      ...
      <form className="flex flex-col gap-3" onSubmit={(event) => { event.preventDefault(); const result = composeScenes(memo, pairs, newId); if (result.missing.length > 0) setMessage(copy.memory.missing(result.missing)); else setStory({ ...story, scenes: result.scenes.map((scene, index) => ({ ...scene, locationId: locations[index]?.id })) }); }}><label className="t-ui flex flex-col gap-2">{copy.memory.input}<input className="field mono" maxLength={5000} value={memo} onChange={(event) => { setMemo(event.target.value); }} /></label><button className="btn self-start" type="submit">{copy.memory.compose}</button></form>
      ...
      <ol className="story-sequence">{story.scenes.map((scene, index) => {
        ...
        return <li className="story-scene" key={scene.id} draggable onDragStart={() => { dragging.current = index; }} onDragEnd={() => { dragging.current = undefined; }} onDragOver={(event) => { event.preventDefault(); }} onDrop={(event) => { event.preventDefault(); if (dragging.current !== undefined) moveScene(dragging.current, index); dragging.current = undefined; }}>
          <div className="control-row"><h3 className="t-ui">{copy.memory.scene(index + 1)} · <span className="mono">{scene.pairId}</span></h3><div className="control-row ml-auto"><button className="btn" type="button" aria-label={copy.memory.up} disabled={index === 0} onClick={() => { moveScene(index, index - 1); }}>↑</button><button className="btn" type="button" aria-label={copy.memory.down} disabled={index === story.scenes.length - 1} onClick={() => { moveScene(index, index + 1); }}>↓</button><button className="btn" type="button" onClick={() => { setStory({ ...story, scenes: story.scenes.filter((item) => item.id !== scene.id) }); }}>{copy.common.remove}</button></div></div>
          {image?.asset === undefined ? null : <img src={image.asset} alt={image.text} className="max-h-40 max-w-full object-contain rounded-xl" />}
          <label className="t-ui flex flex-col gap-2">{copy.memory.word}<textarea className="field" rows={2} maxLength={1000} value={scene.text} onChange={(event) => { setStory({ ...story, scenes: story.scenes.map((item) => item.id === scene.id ? { ...item, text: event.target.value } : item) }); }} /></label>
