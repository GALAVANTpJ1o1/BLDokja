...
// Firefox restores a button's dynamic disabled state on reload, before hydration.
// This one native attribute keeps the trigger disabled until SettingsProvider is ready.
const triggerSessionPolicy = { autoComplete: "off" } as const;
...
export function SiteGuide({ invitation = false }: { invitation?: boolean }) {
  ...
  return <>
    {invitation ? showInvitation ? <div className="guide-invitation"><p className="t-meta">{copy.welcome}</p><button ref={trigger} className="btn" type="button" onClick={show}>{index === 0 ? copy.start : copy.resume}</button></div> : null : <button {...triggerSessionPolicy} ref={trigger} id="site-guide-trigger" className="btn" type="button" disabled={!ready} onClick={show}>{copy.title}</button>}
