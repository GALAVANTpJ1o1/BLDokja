...
describe("a highlighted sticker in the net", () => {
  ...
  it("brings the rest of its piece in full colour, keeps every centre coloured, and hides everything else (the reported screen)", () => {
    render(<StickerNet label="Cube after the scramble" cells={cells()} highlight={new Set([slotNamed("UBL")])} hideUnrevealed />);
    const svg = screen.getByRole("img");

    // The buffer piece is green-red-yellow; its three stickers show those colours at full strength.
    expect(paint(svg, slotNamed("UBL"))).toEqual({ fill: "var(--face-f)", opacity: "1" });
    expect(paint(svg, slotNamed("LUB"))).toEqual({ fill: "var(--face-r)", opacity: "1" });
    expect(paint(svg, slotNamed("BUL"))).toEqual({ fill: "var(--face-d)", opacity: "1" });

    // The six centres keep their colour at full strength.
    for (const face of ["U", "L", "F", "R", "B", "D"]) expect(paint(svg, slotNamed(face)), face).toEqual({ fill: `var(--face-${face.toLowerCase()})`, opacity: "1" });

    // Nothing else gives a colour away.
    const shown = new Set(["UBL", "LUB", "BUL", "U", "L", "F", "R", "B", "D"].map(slotNamed));
    for (const s of cube.geometry.stickers.filter((sticker) => !shown.has(sticker.index))) expect(paint(svg, s.index), stickerName(cube.geometry, s.index)).toEqual({ fill: "var(--rule)", opacity: "0.28" });
  });

  it("fades the rest to faint colour rather than blanks when colours may show, and still never fades a centre", () => {
    render(<StickerNet label="Cube after the scramble" cells={cells()} highlight={new Set([slotNamed("UBL")])} />);
    const svg = screen.getByRole("img");
    expect(paint(svg, slotNamed("UFR")).opacity).toBe("0.28");
    expect(paint(svg, slotNamed("UFR")).fill).toMatch(/^var\(--face-[a-z]\)$/);
    for (const face of ["U", "L", "F", "R", "B", "D"]) expect(paint(svg, slotNamed(face)).opacity, face).toBe("1");
  });
  ...
  it("draws every sticker normally, with no rings, when nothing is highlighted", () => {
    ...
    const svg = screen.getByRole("img");
    for (const s of cube.geometry.stickers) expect(paint(svg, s.index).opacity, stickerName(cube.geometry, s.index)).toBe("1");
  });
});
