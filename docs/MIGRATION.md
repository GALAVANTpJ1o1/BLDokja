...
**Status:** plan, awaiting review. Nothing described here is built yet. Per [BRIEF.md](../BRIEF.md) §14, the importer and its tests are built in **Phase 2**, together with the storage layer. The decisions marked **[proposed]** turn into `docs/DECISIONS.md` entries once approved.
...
|---|---|---|---|
| `pair_words` | 678 | Your words for all 552 pairs, with use counts | `letterPairs` (§3.2) |
...
1. **Lossless.**
   - Every source row becomes exactly one target record.
   - Every source column value is kept verbatim, in a `legacy` snapshot that later edits never touch.
   - Row counts in and out must match or the import aborts.
...
3. **No silent normalisation.**
   - Text is stored exactly as found: no trimming, re-casing, Unicode normalisation or typo fixing.
   - Cleanup happens later, in the app, visibly and with undo.
...
type PairImage = {
  id: string;            // legacy: "legacy:pair_words:32"; new: random UUID
  text: string;          // verbatim
  uses: number;          // legacy `count`; see AUDIT C1 for what it does and doesn't mean
  legacy?: {             // immutable snapshot of the source row
    table: "pair_words";
    id: number; pair: string; word: string; count: number;
  };
};
```
...
- **The original score is kept as it was.** If a better scorer is added later (AUDIT C3 shows positional scoring under-scores 17 attempts), it stores a *derived* score next to the original.
...
**Counts**
6. Records emitted must equal the source row count for each table.
7. The set of emitted legacy IDs must equal the set of source IDs, which proves no row was skipped or doubled.

**Envelope**
8. The generated envelope must pass the same Zod schema stage 2 uses.
...
**Counts and totals**
- Counts per table.
- Pairs covered, e.g. 552 / 552 distinct-letter pairs.
- Sum of uses.

**Things worth reviewing** (the lists in AUDIT §4.6)
- Primaries that were decided by a tie-break (89).
- Probable typos and spacing duplicates.
- Words shared by more than one pair (13).
- Words containing digits or punctuation (these matter for TTS).

**Known gaps**
- Legacy memo attempts whose stored score is lower than an LCS alignment would give (17).
- Deleted-ID gaps: informational only.

The report **suggests**; it doesn't **change** anything. Merging `engish` into `english` is a deliberate action in the app's library-health view, with undo.
...
5. **Check in the UI.**
   - Check that the grid shows 552 pairs.
   - Spot-check the sample rows from AUDIT §4.7.
...
- Typo and spacing-variant pairs (`engish`/`english`, `townhall`/`town hall`).
...
- Beyond the synthetic suite, it also asserts the golden numbers from AUDIT: 678 / 552 / 51 / 4 rows, source SHA-256, 89 tied pairs, 13 shared words.
...
| # | Test | Passes when |
|---|---|---|
| 1 | **Count parity** | Records out equal rows in, for each table |
| 2 | **Identity bijection** | The set of legacy IDs in the export equals the set of source IDs, with no duplicates |
| 3 | **Value fidelity** | For every source row and column, the `legacy` snapshot value `Object.is`-equals the SQLite value, and its JS type matches the storage class (`integer` → `Number.isInteger`, `real` → finite number, `text` → string) |
| 4 | **Derived fields** | `images` are ordered `count DESC, word ASC`; `text === legacy.word`; `uses === legacy.count`; `at === created_at.replace(" ", "T") + "Z"` |
...
- **Cleaning the data.** Typo merging, primary selection and shared-word disambiguation all happen in the new app, through the library-health view, as your own decisions.
- **The Android client and the Flask API.** No data lives there.
