...
- The checkpoint keeps what was typed. For letter answers (traces, 4x4x4 wing and corner traces, commutator cases, single letters) it sets your letters against the right ones in a table, target by target, with a cross on each that differs (not colour), a sentence for the first difference ("It first goes wrong at target 4: you typed C, the right letter is V"), and dashes where a target was never typed or nothing was expected. Long memos wrap onto tables of eight so nothing scrolls sideways at 380px. Other kinds (setups, parity, mistakes) now say "You typed: …" next to the right answer, with radio choices shown as words.
