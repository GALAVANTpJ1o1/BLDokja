...
- **UI verification gap, stated plainly.** `pnpm build` succeeds and `/account` prerenders (confirmed by inspecting the static output directly), and `pnpm typecheck`/`lint`/`test` are all clean, including new tests for the pure logic in `account.ts` and the storage-namespacing logic in `storage-client.ts`. What is **not** verified: actually loading `/account/` in a live browser and exercising sign-up, sign-in, and the forgot-password flow end to end through the React UI itself (as opposed to the underlying functions, which D-063 does verify directly). This session's browser tool could not reach `localhost` (a sandboxing boundary, not a one-off permission prompt -- confirmed by starting the dev server directly and checking the port was genuinely listening before the navigation was still refused), and the Claude in Chrome extension wasn't installed/signed in on this machine when offered as an alternative. The owner should run `pnpm dev` and click through `/account/` before this is considered done, not just built.

## D-063 · Both Edge Functions deployed and verified end-to-end against the live project

**Status:** verified (2026-09-18). Closes out D-055's and D-062's "untested end-to-end" caveats for the mechanisms themselves (the React UI driving them is still the one open item, per D-062's last point).

- **`supabase secrets list` confirms the exact env var naming** for this project: `SUPABASE_ANON_KEY` and `SUPABASE_SERVICE_ROLE_KEY` (the legacy names, checked first in both functions) are auto-provisioned *alongside* the newer plural forms, `SUPABASE_PUBLISHABLE_KEYS` and `SUPABASE_SECRET_KEYS` -- not instead of them. Both functions' fallback chains had a naming bug (checking a nonexistent singular `SUPABASE_SECRET_KEY`/`SUPABASE_PUBLISHABLE_KEY`) that made no practical difference here (the primary legacy names already matched), but is fixed to check the real plural names, in case the legacy ones are ever retired.
- **`redeem-recovery-code` had to be deployed with `--no-verify-jwt`.** This project's anon/publishable key (`sb_publishable_...`) is not a JWT (confirmed earlier: one segment, not three) -- Supabase's platform-level JWT gate would reject a call authorized only by that key before it ever reached the function's own code, which is exactly how this function is meant to be called (by someone with no session at all). `delete-own-account` keeps the default (JWT required): confirmed live that a garbage bearer token is refused by the *platform* itself (`UNAUTHORIZED_INVALID_JWT_FORMAT`) before reaching this function's own `auth.getUser()` check -- two independent layers, not one.
- **The full recovery flow, verified live, in order:** sign up → set a recovery code → redeem with the *wrong* code (refused) → redeem with the *right* code (succeeds) → the *old* password now fails to sign in, the *new* one succeeds (proves the password genuinely changed, not just that the function returned `ok: true`) → redeeming the same code again is refused (used codes don't work twice).
- **`delete-own-account`, verified live:** a garbage token is refused at the platform level; the real call with the test account's own session succeeds and actually removes the account (its username becomes available again); calling it a second time with the same (now-revoked) token is refused, not silently repeated. The Storage-cleanup branch ran (found zero objects, since this test account never uploaded an image) but the *recursive listing* logic specifically -- walking into a pair-id subfolder to find nested image files -- remains unexercised against a real uploaded image; that needs the actual upload path built (M4) before it can be tested for real.
- No manual test-data cleanup was needed this time: `delete-own-account` did its own, which is exactly the feature working as intended rather than a coincidence.

## D-064 · Operational gotcha: `@bld/storage` resolves through its built `dist/`, not live source -- rebuild it after every source change

**Status:** noted (found this session, 2026-09-18). Process guidance, not a design decision, but expensive to relearn if forgotten.

- **What happened.** `packages/storage/package.json`'s `exports` field points `@bld/storage` at `./dist/*.js` (compiled output), not at `src/`. After adding `outbox`/`enqueueOutbox`/`dequeueOutbox` and `DEXIE_VERSION` to `packages/storage/src` (D-057), a new test in `apps/web` (`sync/events.test.ts`) failed with `storage.enqueueOutbox is not a function` -- `dist/storage.js` genuinely had zero occurrences of `enqueueOutbox` at that point, confirmed by grepping the compiled file directly.
- **The confusing part, recorded honestly rather than papered over:** an *earlier* `apps/web` test (`storage-client.test.ts`) exercising the same new methods had already passed in this session, and every `pnpm typecheck` run up to that point was clean too, which should have been impossible against a `dist/index.d.ts` that hadn't been regenerated. The exact mechanism was not conclusively tracked down (a plausible guess is that some earlier command incidentally rebuilt it, or TypeScript's `moduleResolution: "Bundler"` resolved types more leniently than the runtime `exports` field resolved the `.js` -- neither was confirmed). What matters operationally is what fixed it, and that it now reproduces reliably in the "stale" direction.
- **The fix:** `pnpm --filter @bld/cube-engine --filter @bld/storage --filter @bld/srs --filter @bld/analytics build` (exactly what the root `package.json`'s own `dev` script already does before starting `next dev`) rebuilds `dist/` from current source. After doing this, all 770 repo tests, typecheck, lint (aside from the pre-existing, unrelated D-05x navigation-cube.tsx issue) and a fresh `pnpm build` all passed cleanly, re-verifying `/account` and every other route against the corrected package.
- **Going forward:** run the root-level `pnpm build` (or the four-package filtered build above) after any change under `packages/*`, before trusting an `apps/web`-scoped `pnpm --filter @bld/web test/build` result. The root `pnpm test`/`pnpm typecheck`/`pnpm lint` scripts run each package's own script in workspace order, which is *not* the same thing as rebuilding `dist/` -- only an explicit `build` does that.

## D-065 · M4 status: events/pairs/settings sync, migration and conflict resolution are live; images are not

**Status:** milestone summary (2026-09-18), not itself a design decision -- an index of what M4 actually delivered, so the gap that's left is explicit rather than implied.

**Built, tested (220 `apps/web` tests + the M2/M3 suites, all green) and wired into a running provider:**
- Event sync: `lib/sync/events.ts` (outbox push with `upsert(ignoreDuplicates)`, cursor-based pull, Zod-validated at the boundary). `useEvents()`'s `append()` -- the one call site every trainer already goes through -- enqueues the outbox entry.
- Letter-pair sync: `lib/sync/pairs.ts`, full reconciliation rather than an outbox (D-060-adjacent reasoning: the dataset is small and bounded, and there's more than one write call site, found by grepping -- a full compare can't miss one, an outbox hook could). Conflicts are detected (concurrent edit, remote-delete-vs-local-edit) and now resolvable (`resolveLetterPairConflict`, "keep mine" / "use synced").
- Settings sync: `lib/sync/settings.ts`, one round trip through the already-built `merge_settings` RPC, merged onto the existing local record rather than replacing it.
- Orchestration: `lib/sync/orchestrator.ts` runs all three concurrently and reports one of the plan's own required states; `lib/sync/scheduler.ts` adds `navigator.locks`-based multi-tab coordination; `components/sync/sync-provider.tsx` actually runs it -- on sign-in, every 60s, on tab foreground, and on reconnect -- and exposes status/conflicts/`syncNow` to a new section on the account page.
- Guest migration: `lib/sync/migration.ts` copies the guest's local data (read-only access to the untouched `"bldokja"` database) into the account's own storage via the existing `importData()` merge, seeds `syncFieldUpdatedAt` so the copied settings actually have something to push, then runs a cycle. Wired into a `PostAuthFlow` that also fixed a real bug: `AccountProvider`'s auto-reload on sign-in would otherwise race the one-time recovery-code reveal.

**Simplifications made explicitly, not by omission:**
- **Retry is a fixed 60s interval, not exponential backoff.** The plan asks for "retry with backoff"; at this scale (~80 users, ≤20 concurrent) a fixed interval plus the foreground/reconnect triggers was judged sufficient rather than adding backoff-state bookkeeping for a load profile that doesn't need it. Revisit if real usage shows repeated-failure hammering.
- **Letter-pair conflict resolution is pick-a-side, not field-level merge.** "Keep mine" / "use synced" is the whole UI; there's no per-field diff view.

**Image sync: now built, and live-verified.** `lib/sync/pair-images.ts` uploads an inline base64 `asset` to the private `letter-pair-images` bucket at push time (path `<account_id>/<pairId>/<imageId>.<ext>`, matching D-056's convention) and replaces it in what's actually sent to `sync_letter_pairs.data` with a small `{assetStoragePath, assetMime}` reference; the reverse happens on pull, reconstructing the inline `asset` before the result is validated against `LetterPairSchema` (which would otherwise reject the reference shape as an unknown field -- `.strict()` has no concept of it). `reconcileLetterPairs()`'s three conflict-reporting branches all route through this same inflation step now, not just the plain-apply path: an earlier draft cast `row.data as LetterPair` directly in those branches, which would have shown broken/un-inflated data for any conflicting pair that happened to have an image.

- **Verified live** (a fresh browser tab, two throwaway accounts, real Storage calls, cleaned up via `delete-own-account` afterward): uploading a real 1x1 PNG as one account, reading it back as that same account, a second account being refused both read (looks like a 404, not a 403 -- Storage doesn't reveal that the object exists at all) and write access into the first account's folder. `delete-own-account`'s Storage-cleanup step ran against a real uploaded object for the first time here (previously only exercised with zero objects) and completed without error, though there was no independent way to *prove* the object was actually removed without service-role access -- reasonable confidence, not full certainty, on that specific point.
- **Failure handling, unit-tested with a fake bucket:** an upload failure falls back to sending the row with the image still inline (the row is just bigger that once, never silently dropped); a download failure causes that one row to be skipped for the cycle (not corrupted, not applied) rather than crashing the whole reconciliation.
- With this, M4 as originally scoped is functionally complete. The two documented simplifications above (fixed-interval retry, pick-a-side conflict resolution) still stand.

## D-066 · M5: activity calendar, daily goal and leaderboards are live and guest-available where the data allows

**Status:** milestone summary (2026-09-18).

- **Activity calendar and streaks are fully guest-available.** `activityDays()`/`streaks()` (`packages/analytics`) read only the local event log, mirroring `leaderboard_streaks()` SQL (D-060) exactly so a signed-in device's own view and the opted-in public board never quietly disagree about what "current streak" means. The calendar UI (`progress-view.tsx`) follows the mastery.tsx accessible pattern -- an opacity ramp plus a size-varying mark, never colour alone -- rather than the classic GitHub green scale, which DESIGN.md's own guardrails rule out anyway.
- **Two react-hooks lint patterns worth remembering**, both hit building this milestone:
  - `Date.now()`/`Math.random()` cannot be called during render, including inside a `useMemo` callback -- but a `useState(() => Date.now())` *lazy initializer* is exempt (runs once, at mount), which is the pattern used throughout rather than an effect-plus-setState.
  - Calling `setState` synchronously at the top of an effect body (e.g. "reset to loading, then fetch") is flagged by `react-hooks/set-state-in-effect` as an avoidable extra render. Fixed everywhere it came up (the leaderboard page, the daily-goal settings controls) by keying state to *what it answers* (`{ board, rows }`) and deriving "still loading" by comparing that key to the current request, or by using local override state instead of an effect-seeded one -- never by wrapping the call in a `setTimeout` just to dodge the linter.
- **Leaderboards read five security-definer RPCs** (`supabase/migrations/20260918100006_leaderboards.sql`, D-059) plus the monthly archive, through a narrow injectable `LeaderboardClient` interface matching the existing `SettingsSyncClient` shape (D-056-adjacent). Results are Zod-validated with `z.coerce.number()` on `rank`/`value` rather than assumed to already be JS numbers: a couple of these functions declare them `bigint`, and PostgREST's JSON encoding of that type wasn't verified live in this session (no seeded leaderboard data existed to check against) -- coercion is the safe assumption either way, not a real cost.
- **The leaderboard-opt-in checkbox in the account page (built in M3) still doesn't seed its initial state from the server** -- it starts unchecked/empty and only writes on submit, a pre-existing gap noted here rather than fixed, since it predates this milestone and touching it wasn't in scope.

## D-067 · M6: loading cube, SEO/launch metadata, breadcrumbs, Contact/Privacy -- and one plan item that was never built

**Status:** milestone summary (2026-09-18).

- **The loading cube never invents a move.** `LoadingCube` (`components/cube/loading-cube.tsx`) idles through `randomMoveSequence()`/`verifiedMoves()` (`packages/cube-engine`) -- the same engine-tested building blocks `NavigationCube` already uses for its routes -- rather than any hand-written sequence, satisfying the project's own "never write a cube algorithm from memory" rule even for a purely decorative animation.
- **A root-level `alternates.canonical` would have been wrong**, not just redundant: Next.js merges unset metadata keys from the parent layout, so setting it once at the root would have made every page that doesn't override it claim canonical URL "/" -- telling search engines every route is a duplicate of the homepage. Fixed by giving every route (all static pages, plus a per-lesson `generateMetadata`) its own explicit canonical instead. Worth remembering before adding any new route: it needs its own `alternates.canonical`, there is no safe site-wide default.
- **`https://bldokja.pages.dev` is hardcoded** in `layout.tsx`'s `metadataBase`, `sitemap.ts` and `robots.ts`, ahead of the Cloudflare Pages project actually existing (M8) -- the domain confirmed in `docs/DEPLOY.md`. If the final domain ever differs, all three need updating together.
- **Privacy page describes only what's actually shipped, checked against the real code, not the original v2 plan.** Two planned pieces were never built and the page doesn't claim them: email verification (moot -- D-054 already made email unnecessary) and a Cloudflare Turnstile anti-bot check on sign-up (grepped for at write time; genuinely absent). If Turnstile is added later, the privacy page needs a matching update.
- **Breadcrumbs replaced, rather than duplicated, the lesson page's existing "back to path" link** -- same reasoning as everywhere else in this session: one navigation pattern per concern, not two competing ones on the same page. Top-level pages (`/practice/`, `/learn/` themselves) don't get a breadcrumb trail at all; "Home > Practice" on the Practice page would just repeat the heading.
- **`next/og`'s `ImageResponse`, `sitemap.ts` and `robots.ts` all need `export const dynamic = "force-static"`** under `output: "export"` -- discovered by the build failing on each in turn, not anticipated. No per-request server exists to render them on demand in a static export, so this is a hard requirement, not a style choice.

## D-068 · M7 status: what is verified, and what is written but has never been run

**Status:** milestone summary (2026-09-18).

- **Verified by running (all green):** engine, storage, srs, analytics and web unit suites; typecheck; lint; production build. New this milestone: DST, midnight and timezone cases for the activity calendar and streaks (`packages/analytics/test`, plan §K scenario 16 -- New York spring-forward and fall-back days, a zone east of UTC, and "today" judged in the reader's own zone), and the leaderboard client's validation (`apps/web/src/lib/leaderboards.test.ts`: bigint-as-string values, an extra `username` field rejected, error versus empty board).
- **Update:** `e2e/launch.spec.ts` has since been run by the owner (chromium and mobile, against `pnpm dev`): 20 of 22 passed, the other two were a test-selector bug (Next emits two identical `noindex` tags on the 404 page), fixed. `accounts.spec.ts` has still never run. Original note follows.
- **Written but never run:** `e2e/launch.spec.ts` (contact, privacy, 404, canonical/description/OG, robots and sitemap, breadcrumbs, the 5BLD label, guest settings surviving a reload) and `e2e/accounts.spec.ts` (sign-up and deletion, guest/account isolation, two-context settings sync). The authoring session could not reach localhost from any tool -- not the browser pane, not `curl` -- so neither file has executed once. Each says so in its header. Expect selector or timing fixes on first run; a failure there is more likely a test bug than an app bug until proven otherwise.
- **`accounts.spec.ts` creates real accounts on the live Supabase project**, so it is skipped unless `BLD_E2E_ACCOUNTS=1`, runs on Chromium only, prefixes every username `e2e-`, and deletes what it creates (a test that fails midway can leave one behind).
...
**Verified live after the redeploy** (deployment `9e3dcf1f`, commit `f34a351`, Production): `/opengraph-image` returns 200, `image/png`, 34,192 bytes with a valid PNG signature, alongside `nosniff`; the manifest, sitemap, robots and service worker types are unchanged; client-side navigation between Learn, Practice, Progress and Account stays in the same document (no full reloads); a browser holding the first deploy's service worker finds the new one, precaches all 622 files under the new cache name and leaves it `waiting` while the old one stays active, as designed.
...
- **Bug.** The opt-in checkbox and display-name box in `account-view.tsx` started unchecked and blank whatever the account held. Someone already opted in who opened the page and pressed save (to change the name, say) sent `leaderboard_opt_in: false` and silently dropped off the boards. The button was labelled "Leaderboards" (the section title) and the confirmation said "Account" (a different section's title).
- **Fix.** `getLeaderboardSettings()` reads the account's own `profiles` row (the `profiles_select_own` policy already allows it, so no migration) and Zod-validates it (`parseLeaderboardSettings`). The form is now its own component, `LeaderboardSection`: disabled while loading, prefilled from the stored values, and if the values can't be read it says so and refuses to save rather than writing guessed defaults over the real ones. A blank name still means "keep the current one"; names are trimmed. New copy: the button reads "Save leaderboard settings", the confirmation "Leaderboard settings saved."
- **Tests.** Four component tests (stored values shown and saved untouched, name-only edit keeps the opt-in, blank name keeps the name, unknown settings block saving) and two for the parser. Reintroducing the original bug (ignoring the stored opt-in) makes exactly the two tests about it fail. `e2e/accounts.spec.ts` gained a fourth scenario against the real database: a new account is not opted in and starts as `solver-xxxxxx`, opting in and renaming survive a reload, and saving untouched afterwards does not undo it. That is the only check that exercises the real row-level-security read. It opts a throwaway account into the public boards for a few seconds; a new account has no activity, so it should not appear in a ranking.

## D-075 · Unhandled `InvalidStateError` from page transitions in a hidden tab

**Status:** fixed (2026-09-19).

Found on the live site while checking client-side navigation from the built-in browser pane, which is hidden while it is driven. Each link click logged `Uncaught (in promise) InvalidStateError: Transition was aborted because of invalid state`.

- **Cause.** `runPageTransition` awaited only the view transition's `finished` promise. Per the View Transitions spec, `ready` rejects with exactly this error whenever the browser skips the animation, most commonly because the document is hidden. The navigation itself still runs and `finished` still resolves, so nothing was ever broken for a reader: the effect was console noise from a rejection nothing listened to.
- **Fix.** `ready` now has a no-op catch. The interface gained an optional `ready`; the fake document in the tests did not model it, which is why nothing caught this before.
- **Test.** A skipped animation (`ready` rejecting, `finished` resolving) must still navigate, clear `data-transition`, and leave nothing unhandled. Vitest fails a run on an unhandled rejection, and did before the fix (all tests green, run red, `Unhandled Rejection: InvalidStateError`).
...
- **Fix.** `AccountView` tracks that an auth call is in flight (`onAuthenticating` from the form) and keeps the form on screen, busy, until the call returns and `PostAuthFlow` takes over; a failed attempt clears it. A regression test holds `signUp()` open after the auth state flips and asserts the signed-in page never appears, before or after release. It failed before the fix.
- **Spec.** `finishPostAuth` retries "click Skip if it is there, then expect signed in" as one unit (`expect(...).toPass`) instead of check-then-act, so it stays correct if the dialog arrives late for any other reason.
...
**Cause.** Not the engine: the stickering masks were already correct (checked by reading the live 3D player's scene, where the trace trainer had the whole piece and all six centres lit). `Cube` lit exactly the slots a caller named, and callers disagreed: the practice trace trainer wrapped its highlight in `piecesOf()` and named the centres through `revealOnly`; `TraceWalk` (lessons 6 and 7), the first-solve memo and the algorithm library passed bare sticker names. Whether a learner saw a usable cube depended on which screen they were on.

**Decision.** The rule lives in `Cube` and `StickerNet`, so no caller has to remember it:
- Naming a sticker lights its whole piece in full colour. The rest of a piece is found **by position** (the slots on the same cubie), not by which piece is sitting there: a corner's stickers never separate, whereas a 4x4x4's same-coloured x-centres share one identity in the engine and would otherwise have lit each other (found by a failing test). Single-sticker pieces have no "rest".
- The fixed centres (a slot named with one face letter: the six on a 3x3x3, and on a 5x5x5) are never dimmed, greyed or hidden, in both renderers and in the text description. A 4x4x4 has no fixed centre; its x-centres are pieces to memorise and are left alone.
- `dim` is **not** used for the rest of a piece. It is half brightness in linear light (about 73% as displayed), and stickers on hidden faces appear only as floating hint facelets at 50% opacity, so a dimmed piece would be near-illegible exactly where it matters. The rest of the piece is drawn at full colour.
- Which sticker is being asked about: the net draws a ring on it (a shape cue, dark inside light so one shows on any colour). The 3D player cannot draw a ring, so the trace prompt now names the sticker's face ("Take its sticker on the Up face…", in all four voices), and the trace trainer passes the single sticker it means, not the whole piece.
- The 3D rule is a pure function, `playerMask`, tested against the real engine on the reported scramble, including that naming one sticker of a piece gives the same mask as naming all three. The engine gained `pieceOfSticker` and a `piece` on every `SlotView`.

**Memo feedback.** A wrong memo used to show only the right answer, so there was no way to see where it went wrong. Now:
- The checkpoint keeps what was typed. For letter answers (traces, 4x4x4 wing and corner traces, commutator cases, single letters) it sets your letters against the right ones in a table, target by target, with a cross on each that differs (not colour), a sentence for the first difference ("It first goes wrong at target 4: you typed C, the right letter is V"), and dashes where a target was never typed or nothing was expected. Long memos wrap onto tables of six (measured on the live page: eight fit from about 375px but overflow a 320px phone), each in a scrollable wrapper, so the page never scrolls sideways. Other kinds (setups, parity, mistakes) now say "You typed: …" next to the right answer, with radio choices shown as words.
...
## D-088 · The letter library scrolls by dragging, and pair images can carry a picture

**Dragging.** The 24 x 24 grid is wider than the screen, and its scrollbar is at the bottom of the grid, far from where you are looking. `useDragScroll` (`components/ui/use-drag-scroll.ts`) makes a mouse press-and-drag scroll the grid sideways and the page up or down, with an open-hand cursor that closes while dragging (`.drag-scroll` in `styles/primitives.css`). Only the mouse: fingers already scroll, and the keyboard is untouched. A press has to move 5 px before it counts as a drag, so a plain click still opens a cell, and the click that ends a drag is swallowed so it cannot open one. Applied to the letter-pair grid only; the 3-style case grid could use the same one line.

**Pictures.** A pair image is a word, and its stored shape already allowed a validated raster picture (`asset`, a PNG/JPEG/WebP data URL up to 2.8 MB, synced by `lib/sync/pair-images.ts`), but only the Memory workspace page offered it. The pair editor now has a picture control on each image row, using the same `readRaster` check (real file signature, PNG/JPEG/WebP, 2 MB), a thumbnail in the editor and in the grid cell, and a remove button. Merging two words no longer drops a picture (`renameOrMerge` keeps the kept word's, else the other's). No new storage format or sync path.

## D-089 · Settings writes are built from the settings as they are at write time

Choosing a corner buffer sticker and then an edge buffer sticker in quick succession lost the second choice on a slow device: each change was built from a copy of the stored settings taken at render time, so the later write replaced the whole `buffers` object with a stale one (found by `e2e/buffer-orientation.spec.ts` on the live site). `update` now also accepts a function that receives the settings inside the write transaction, and the buffer picker uses it, so writes cannot clobber each other. Other callers still pass a plain patch.

**Addendum (D-088): pictures in "Find a word".** The Find a word tab (`PairDiscover`) also takes a picture directly: a file chooser (not a text box) beside the word field, with a thumbnail and a remove button, using the same `readRaster` check. The picture goes onto the image being added, or onto the pair's existing image when the word is already there. A picture with no word is named after its file (or the pair), and can be renamed in the editor. The word field stays for the word itself; a picture never has to be typed.
