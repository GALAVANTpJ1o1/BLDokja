...
**Not built at all: image sync.** `PairImage.asset` (the base64-embedded picture) never leaves the local device yet -- letter pairs sync their text fields, but an image attached to one does not round-trip to another device. The Storage bucket, path convention and RLS policies already exist and were verified in D-060/D-063; what's missing is the client-side upload/download/caching path itself. This is the one remaining piece of M4 as scoped in the original plan.
